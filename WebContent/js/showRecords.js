	...
	if(number==z && flag ==1){
		...
		for(var j = d-1;j>=0;j--){
			if(recd[p+j].type == "I"){
				...
				a.append("<div><img src="+ recd[p+j].headUrl + " class=\"img-circle\"><span class=\"label label-info\">"+recd[p+j].staffName+"</span>"+new Date(recd[p+j].createdDt).pattern("yyyy-MM-dd EEE hh:mm:ss")+"<div class=\"amessage\" id=\"singleRec"+j+"\"><pre class=\"prettyprint\">"+recd[p+j].content+"</pre></div><br></div>");
		...
		for(c in recd){
			if(recd[p].type == "I"){
				...
				a.append("<div><img src="+ recd[p].headUrl + " class=\"img-circle\"><span class=\"label label-info\">"+recd[p].staffName+"</span>"+new Date(recd[p].createdDt).pattern("yyyy-MM-dd EEE hh:mm:ss")+"<div class=\"amessage\" id=\"singleRec"+p+"\"><pre class=\"prettyprint\">"+recd[p].content+"</pre></div><br></div>");
